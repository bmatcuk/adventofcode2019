...
//!
//! Each moon has a 3-dimensional position (x, y, and z) and a 3-dimensional velocity. The position
//! of each moon is given in your scan; the x, y, and z velocity of each moon starts at 0.
//!
//! Simulate the motion of the moons in time steps. Within each time step, first update the
//! velocity of every moon by applying gravity. Then, once all moons' velocities have been updated,
//! update the position of every moon by applying velocity. Time progresses by one step once all of
//! the positions are updated.
//!
//! To apply gravity, consider every pair of moons. On each axis (x, y, and z), the velocity of
//! each moon changes by exactly +1 or -1 to pull the moons together. For example, if Ganymede has
//! an x position of 3, and Callisto has a x position of 5, then Ganymede's x velocity changes by
//! +1 (because 5 > 3) and Callisto's x velocity changes by -1 (because 3 < 5). However, if the
...
//! own position. For example, if Europa has a position of x=1, y=2, z=3 and a velocity of x=-2,
//! y=0,z=3, then its new position would be x=-1, y=2, z=6. This process does not modify the
...
//! the sum of the absolute values of its velocity coordinates. Below, each line shows the
...
//!
//! What is the total energy in the system after simulating the moons given in your scan for 1000
//! steps?
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for _ in 0..1000 {
        ...
        for i in 0..(num_moons - 1) {
            ...
            for moon2 in right.iter_mut() {
                moon1.update_velocity(moon2);
                moon2.update_velocity(moon1);
            }
            moon1.update_position();
        }
        moons[num_moons - 1].update_position();
    }

    let result: usize = moons.iter().map(|moon| moon.energy()).sum();
