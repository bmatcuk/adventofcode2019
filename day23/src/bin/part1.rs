...
//! Controller (NIC) software (your puzzle input). The computers have network addresses 0 through
//! 49; when each computer boots up, it will request its network address via a single input
...
//! over the network by sending and receiving packets. All packets contain two values named X and
...
//! incoming packet queue is empty, provide -1. Otherwise, provide the X value of the next packet;
//! the computer will then use a second input instruction to receive the Y value for the same
//! packet. Once both values of the packet are read in this way, the packet is removed from the
//! queue.
//!
//! Note that these input and output instructions never block. Specifically, output instructions do
//! not wait for the sent packet to be received - the computer might send multiple packets before
        ...
        for i in 0..COMPUTERS {
            ...
            for j in (0..output.len()).step_by(3) {
                if (0..(COMPUTERS as i64)).contains(&output[j]) {
                    ...
                    println!("First Y sent to 255: {}", output[j + 2]);
                    break 'outer;
                }
            }
